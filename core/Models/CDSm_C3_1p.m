...
PURPOSE
    Build a predefined crane model: Single pendulum
%}

classdef CDSm_C3_1p < CDSm_C3
methods
    %**********************************************************************
    % Interface: Create
    %***********************************
    function this = CDSm_C3_1p(varargin)
    ...
    function sys = Build_SystemDescription(this)
        ...
        % Check input
        %***********************************
        mustNotBeZero_2D = [V.mass_K; V.mass_M];
        ...
        % Single pendulum simplification
        params.Create('free', 'theta_p1i').SetIC(V.theta_p1i_IC);
        theta_p2i = V.theta_p2i_IC;
        theta_p2o = V.theta_p2o_IC;

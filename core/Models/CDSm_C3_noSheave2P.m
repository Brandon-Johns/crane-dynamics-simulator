...
PURPOSE
    Build a predefined crane model:
        Pulley with zero-radius sheaves, where the hook and payload are rigidly fixed together
        (like a double pendulum)

NOTES
    Model definition fixed from CraneSim1
    Translation 'L_CD' was in 'T_CD0'. Now moved to 'T_D0D1'
%}

classdef CDSm_C3_noSheave2P < CDSm_C3
methods
    %**********************************************************************
    % Interface: Create
    %***********************************
    function this = CDSm_C3_noSheave2P(varargin)
    ...
    function sys = Build_SystemDescription(this)
        ...
        params.Create('const', 'L_EF').SetNum(0);
        ...
        % START: (NOT COMPLETE) Move system into equilibrium - Calculations only for zero radius sheave version
        %theta_2_IC = CDS_Param_Input('tmp').Set_Selector(V.theta_2_t).q(0);
        %R_A2A3_IC = CDS_T('at', 'z', theta_2_IC);
        %R_A3B_IC  = CDS_T('at', 'z', -V.phi_1);
        %T_BC_IC   = CDS_T('atP', 'z', V.theta_3_eq, [V.L_BC;0;0]);
        %P_CD0_IC  = CDS_T('P', [V.L_CD;0;0]);
        %P_BD_measureWorld = R_A2A3_IC*R_A3B_IC*T_BC_IC*P_CD0_IC;
        %x_BD_measureWorld = P_BD_measureWorld.x;
        %x_EH_measureWorld = V.L_EF + V.L_FG + V.L_GH;
        %theta_3_IC_tiltUp = -asin((1/2)*(x_EH_measureWorld - x_BD_measureWorld)/ V.a_DE_IC);
        %V.theta_3_IC = theta_2_IC - V.phi_1 - theta_3_IC_tiltUp;
        %V.theta_7_IC = theta_2_IC - V.phi_1 + V.theta_3_IC;
        % END: Move into equilibrium
        ...
        % START: 0-radius hook block sheaves simplification
        %   Move theta_3 to end of sheave
        %   Equations using these all modified
        T_BC   = CDS_T('atP', 'z', theta_3_eq, [L_BC;0;0]);
        T_CD0  = CDS_T('atP', 'z', theta_3-theta_3_eq, [L_CD;0;0]);
        T_D0D1 = CDS_T('atP', 'z', -sym(pi/2), [0;0;0]);
        T_CD1  = T_CD0*T_D0D1;
        % END: Simplification
        ...
        % Rope length
        %   This is the ZS version => Don't add extra length otherwise model does not start stationary
        %   This took how long to figure out T_T
        L_rope = a_HB + a_DE;
